				...
				<div class="ibox-title">
					...
					<div class="ibox-tools">
						...
						<ul class="dropdown-menu dropdown-user">
							<li><a  href="javascript:layyer('<?php echo U('Admin/Products/add');?>','添加产品')">添加产品</a></li>
						</ul>
					...
					<div class="row">
						<form name="searchform" action="" method="get" >
							...
							<select name="searchtype" class="input-sm form-control input-s-sm pull-left" style="width: 40%">
								<option value='1' <?php if($_GET['searchtype']==1 ): ?>selected<?php endif; ?> >名称</option>
							</select>
						...
						<form name="myform" id="myform" action="" method="post">
							<table class="table table-striped">
								...
								<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
										...
										<td>
											<img src="<?php echo ($vo["thumb"]); ?>" style="width: 77px; height: 55px;">
										</td>
										<td><a href="<?php echo U('/productss/detail',array('id'=>$vo['nid']));?>" target="_blank" title="<?php echo ($vo["tile"]); ?>">
											<span style="" ><?php echo ($vo["title"]); ?></span>
										</a>
										</td>
										<td><?php echo ($vo["cate_name"]); ?></td>
										<td>
											<?php if($vo['istj'] == 1): ?><span class="glyphicon glyphicon-ok" aria-hidden="true"></span>
												<?php else: ?>
										...
										<td>
											<a href="javascript:layyer('<?php echo U('Admin/Products/edit',array('nid'=>$vo[nid]));?>','修改产品')">修改</a>&nbsp;|&nbsp;<a href="<?php echo U('Admin/Products/del',array('nid'=>$vo[nid]));?>" onclick="Delete(this);return false;">删除</a>
										</td>
									</tr><?php endforeach; endif; ?>
								</tbody>
							</table>
							<div>
								<input type="button" class="btn btn-primary" value="排序"  style="cursor:pointer" onclick="myform.action='<?php echo U('Admin/Products/listorder',array('dosubmit'=>1));?>'; myform.submit();"/>
