...
public function add() {
		...
		if (isset ( $_POST ['dosubmit'] )) {
			...
			if ($status) {
				$this->success("恭喜您,友情链接添加成功！",'/Admin/Link/index');
			} else {
				$this->error("对不起，友情链接添加失败");
			}
		}else{
	...
	public function edit() {
		if(isset($_POST['dosubmit'])){
			...
			$datas['link_add_time'] = time();
			  ...
			  $status = $this->Link->where(" link_id = '$_POST[link_id]' ")->save($datas);
			 
			if ($status) {
				$this->success("恭喜您,友情链接修改成功！",'/Admin/Link/index');
			} else {
				$this->error("对不起,友情链接修改失败");
			}
		 ...
		 $link_id = isset($_GET['link_id'])?trim($_GET['link_id']):'';
		  ...
		  if(is_array($linkids)){
				 foreach ($linkids as $id){
					...
					if(!$staus)
					$this->error("对不起,友情链接删除失败！");
				  }
				  $this->success("恭喜您,友情链接删除成功！");
		  }else{
