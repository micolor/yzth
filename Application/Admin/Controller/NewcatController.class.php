...
class NewcatController extends AdminbaseController {
    ...
    $data = M('newcat')->where("parent_id =0 ")->order(" sort_order asc ")->select();
    ...
    function add(){
    	if(isset($_POST['dosubmit'])){
    		...
    		$rel=M('newcat')->add($data);
    		if($rel){
    			$this->success("添加成功！","/Admin/Newcat/index");
    		}else{
    			$this->error("添加失败！");
    		}
    	}else{
    ...
    function edit(){
    	if(isset($_POST['dosubmit'])){
			...
			$data["cthumb"] = $cthumb;
            $data["is_show"] = I("post.is_show");
			$rel=M('newcat')->where("cate_id=$id")->save($data);
    		if($rel){
    			$this->success("修改成功！",'/Admin/Newcat/index');
    		}else{
    			$this->error("修改失败！");
    		}
    	}else{
			...
			// 获取修改的栏目数据
			$cate =M('newcat')->where("cate_id=$id")->find();
    ...
    function delete(){
		...
		if(is_array($cateid)){
			foreach ($cateid as $id){
				if (empty($id)) $this->error("参数错误");
				$finfo = M('newcat')->where("parent_id=$id")->find();
				if ($finfo) {
					$this->error("请先删除相关的下级分类，再删除父级别的分类！");
				}
				$rel = M('newcat')->where("cate_id=$id")->delete();
				if (!$rel) {
			...
			$finfo = M('newcat')->where("parent_id=$cateid")->find();
			if ($finfo) {
				$this->error("请先删除该分类下的子分类！");
			}
			$rel = M('newcat')->where("cate_id=$cateid")->delete();
			if ($rel) {
				$this->success("删除成功！");
			} else {
				$this->error("删除失败！");
    ...
    $listorders  = $_POST['listorders'];
		...
		foreach($listorders as $id=>$listorder){
			 ...
			 M('newcat')->where(" cate_id = '$id' ")->save($da_order);
		}	
        $this->success("排序更新成功！");
