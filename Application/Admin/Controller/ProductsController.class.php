...
class ProductsController extends AdminbaseController
{
    ...
    {
        parent::_initialize();
        $this->catdb = D('products_cat');
        $this->newsdb = D('products');
        $this->ncontentdb = D('products_content');
        $this->db_push = D('push');
    }
    ...
    {
        ajaxsdoimg('/imageuploads/products/', $max_size_val = 2048, $max_width_val = '800');
        exit();
    }
    ...
    public function del()
    {
        ...
        if (is_array($nid)) {
            foreach ($nid as $key => $id) {
                M("products")->where("nid=$id")->delete();
                M("products_content")->where("nid=$id")->delete();
            }
            $this->success("删除操作成功！");
        } else {
            if (!$nid) $this->error('参数传递错误');
            M("products")->where("nid=$nid")->delete();
            M("products_content")->where("nid=$nid")->delete();
            $this->success("删除操作成功！");
        }
                ...
                if ($push_id) {
                    foreach ($id as $new_id) {
                        if ($new_id) {
                            ...
                            if ($an_info['catid']) $cat_info = M('products_cat')->where(array('cate_id' => $an_info['catid']))->find();
                            $info = array();
                            $info['new_id'] = $new_id;
